...
// Global Variable
int qstate; //state of the query 0 IS SUCCESSFUL 1 IS FAILED
MYSQL* conn; //mySQL connection variable
MYSQL_ROW row; //getting the current row of the database
MYSQL_RES* res; //gettig all the values from a database
// Global Variable End
...
public:
    ...
    {
        conn = mysql_init(0); //initializer of mysql_real_connect
        if (conn)
        {
        ...
        conn = mysql_real_connect(conn, "192.168.1.12", "root", "meghana", "cpp_musicstore_db", 0, NULL, 0);
        if (conn)
        {
